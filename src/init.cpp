...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 22337 or testnet: 22336)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 22339)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  The EcoCoin client is probably already running."), strDataDir.c_str()));
    ...
    if (mapArgs.count("-reservebalance")) // ecocoin: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // ecocoin: checkpoint master priv key
    {
    ...
    // TODO: replace this by DNSseed EcoCoin
    AddOneShot(string("127.0.0.1"));
